...
#  =====================
# 
#  Yong Han,    2003/06/10   Original Code
#  P. Stegmann, 2020-11-19   Scheduler script update
#
#------------------------------------------------------------    
...
#      echo "echo End of the job: $TotalJob_count >> optranCoeffJob.log" >> ${jobScript}
      ...
      echo "#SBATCH --job-name=ODAS_Run.%j.out" >> ${jobScript}
      ...
      echo "#SBATCH --output=/scratch/users/pstegmann/GenCoeff/control_ODAS/output.%j.txt" >> ${jobScript}
      echo "#SBATCH --error=/scratch/users/pstegmann/GenCoeff/control_ODAS/output.%j.err" >> ${jobScript}
      # Load the required modules
      echo "module purge" >> ${jobScript}
      echo "module load license_intel/S4" >> ${jobScript}
      echo "module load intel/18.0.3" >> ${jobScript}
      echo "module load hdf/4.2.14 hdf5/1.8.21 netcdf4/4.6.2" >> ${jobScript}
      echo "# " >> ${jobScript}
      echo "# go to the execution directory" >> ${jobScript}
      echo "cd ${RunDir}" >> ${jobScript}
      echo "# execute ${EXE_FILE}" >> ${jobScript}
      echo "srun ${EXE_FILE} > $RunDir/stdout.${ProcSuffix}.txt" >> ${jobScript}
      echo "# kill the stdout file if the excution is successful" >> ${jobScript}
      echo "if [ -f Completion_signal.txt ];then" >> ${jobScript}
      echo "  rm -f stdout.*.txt" >> ${jobScript}
      echo "fi" >> ${jobScript}
