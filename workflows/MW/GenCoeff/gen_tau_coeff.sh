...
#  jobs_sensor=`echo $jobs_sensor $Ngas | awk '{print ($1 * $2)}'`
  ...
  for GasName in $GasName_List; do
    ...
    while [ $TopSeqCh -le $EndChan ]; do
      ...
      mkdir -p $RunDir
      cd ${RunDir}
      ln -s ./GenCoeff/upwelling.avhrr3_n19.TauProfile.nc ./
...
#      echo "#SBATCH --account=star" >> ${jobScript}
      ...
      echo "#SBATCH --output=/scratch/users/pstegmann/CRTM_coef/workdir/MW/GenCoeff/control/output.%j.txt" >> ${jobScript}
      echo "#SBATCH --error=/scratch/users/pstegmann/CRTM_coef/workdir/MW/GenCoeff/control/output.%j.err" >> ${jobScript}
