...
module AbsorberAmount
 ...
 contains
  ...
  subroutine Calc_AbsAmount( p_lev, q_lay, o3_lay, &
    ...
    IF( ANY(p_lev < 0.) ) THEN
      WRITE(*,*) 'Negative level pressure. Calculations aborted. STOP.'
      STOP
    END IF

    IF( ANY(q_lay < 0.) ) THEN
      WRITE(*,*) 'Negative layer water vapor amount. Calculations aborted. STOP.'
      STOP
    END IF

    IF( ANY(o3_lay < 0.) ) THEN
      WRITE(*,*) 'Negative layer ozone amount. Calculations aborted. STOP.'
      STOP
    END IF
    ...
    do Iatm = 1, Natm
      ...
      IF( ANY(absorber < 0.) ) THEN
        WRITE(*,*) 'Negative level absorber amount from Compute_Absorber_Amount. Calculations aborted. STOP.'
        STOP
      END IF 
   ...
   IF( ANY(absamount_lev < 0.) ) THEN
     WRITE(*,*) 'Negative level absorber amount. Calculations aborted. STOP.'
     STOP
   END IF 
...
!    enddo
    ...
    IF( ANY(absamount_lay < 0.) ) THEN
      WRITE(*,*) 'Negative layer absorber amount. Calculations aborted. STOP.'
      STOP
    END IF 
  ...
  subroutine Conv_AbsAmountToLevel( absamount, n_levels, &
    ...
    !--- conversion
    !WRITE(*,*) 'Angle, Profile:', SHAPE(absamount), LOG(absamount(50,2,5)) ! Fails because absamount is negative.
    DO Iatm = 1, Natm
      DO Iang = 1, Nangle
        !WRITE(*,*) Iang, Iatm, absamount(50,Iang,Iatm), abslev(50,Iang,Iatm)
        CALL Conv_AbsAmountToLevel_profile( absamount(:,Iang,Iatm), &
                                            n_levels,               &
                                            abslev   (:,Iang,Iatm)  )
      END DO
    END DO
