...
PROGRAM GenTransCoef
  ...
  '$Id: GenTransCoef.f90 18909 2012-04-24 14:38:18Z yong.chen@noaa.gov $'
  ...
  real(fp_kind),allocatable :: sense_lay  (:,:,:)             ! lay,ang,atm      ! sensitivity
  ...
  ! WRITE algorithm_comments
  WRITE(*,*) '("Training parameters: varying order; max. order = ", &
  ...
  WRITE(*,*) 'Attempting to inquire AtmProfiles.'
  Iflag = AtmProfile_netCDF_InquireFile( InFileName_AtmosProf,       &              
                                     n_Profiles  = Natm,         &       
                                     title       = title,        &       
                                     history     = history,      &       
                                     comment     = comment       )       
  ...
  if( Iflag /= SUCCESS )then                                             
    WRITE(*,*) '### ERROR IN GETTING ATMOSPHERIC PROFILE INFORMATION ###'  
    WRITE(*,*) 'Error flg : ', Iflag                                       
    stop 90                                                              
  endif                                                                  
...
!  ProfileTag = ID_TAG                                                    
  ...
  WRITE(*,*) '# of profiles     : ', Natm                                  
  WRITE(*,*) '# of layers       : ', Nlay                                  
  WRITE(*,*) '# of absorbers    : ', n_AbsInAtmos                          
  WRITE(*,*) 'ID Tag            : ', trim( ID_Tag  )                       
  ...
  Iflag = AtmProfile_netCDF_ReadFile( AtmProfile,                   &
                                      TRIM( InFileName_AtmosProf ), &
                                      Reverse = .True. ) 
  WRITE(*,*) 'AtmProfiles are read from file.'
  IF ( Iflag /= SUCCESS ) THEN
  ...
  Nlay = AtmProfile(1)%n_Layers 
!  Natm = AtmProfile(1)%n_Profiles 
  n_AbsInAtmos=AtmProfile(1)%n_Absorbers
 
  !--- absorber ID and unit                    

  allocate( Absorber_ID      (n_AbsInAtmos) )  
  allocate( Absorber_Units_ID(n_AbsInAtmos) )  
  Absorber_ID(:) = AtmProfile(1)%Absorber_ID(:)
  Absorber_Units_ID(:) = AtmProfile(1)%Absorber_Units_ID(:)
    !--- retrieve absorber profile seq num for wet and ozone
  WRITE(*,*) 'Absorber ID       : ', Absorber_ID
  WRITE(*,*) 'Absorber Units ID : ', Absorber_Units_ID
  !WRITE(*,*) 'Absorber Units    : ', ( trim(ATMPROFILE_ABSORBER_UNITS_NAME(Absorber_Units_ID(Iabs))) // ' ', Iabs=1, n_AbsInAtmos )
  WRITE(*,*) 'Absorber Units    : ', Absorber_Units_ID  
    ...
    enddo
    WRITE(*,*) '### ERROR IN FINDING WATER VAPOR IN ABSORBER ID LIST ###'
    stop 90
1100 continue
    ...
    enddo
    WRITE(*,*) '### ERROR IN FINDING OZONE IN ABSORBER ID LIST ###'
    stop 90
1200 continue

    WRITE(*,*) 'WV index #        : ', IabsWet_AtmosProf
    WRITE(*,*) 'O3 index #        : ', IabsOzn_AtmosProf
    ...
    if( Absorber_Units_ID(IabsWet_AtmosProf) /= UnitID_HITRAN_ppmw .and. &
      ...
      WRITE(*,*) '### ERROR IN ATMOS DATA ###'
      WRITE(*,*) 'Unit of WV is not acceptable.'
      stop 90
    endif

    if( Absorber_Units_ID(IabsOzn_AtmosProf) /= UnitID_HITRAN_ppmw )then
      WRITE(*,*) '### ERROR IN ATMOS DATA ###'
      WRITE(*,*) 'Unit of O3 is not acceptable.'
      stop 90
    endif
    
  !--- allocation

  allocate( p_lev (0:Nlay,Natm) )
  allocate( p_lay (  Nlay,Natm) )
  ...
  profile_loop: DO Iatm = 1,Natm
    P_lev(0:NLay,Iatm)    = AtmProfile(Iatm)%Level_Pressure(0:NLay)
    p_lay(:,Iatm)         = AtmProfile(Iatm)%Layer_Pressure(:)
    t_lay(:,Iatm)       = AtmProfile(Iatm)%Layer_Temperature(:)
    !WRITE(*,*) P_lev(0:NLay,:)
    o3_lay(:,Iatm) =  AtmProfile(Iatm)%Layer_Absorber(:,IabsOzn_AtmosProf) 
  END DO profile_loop 
  if( Absorber_Units_ID(IabsWet_AtmosProf) == UnitID_HITRAN_gpkg )then
    DO Iatm = 1,Natm                                            
      q_lay(:,Iatm) = AtmProfile(Iatm)%Layer_Absorber(:,IabsWet_AtmosProf)
    END DO                       
  else   
    do Iatm = 1, Natm
     CALL ppmv_to_mr( AtmProfile(Iatm)%Layer_Absorber(:,IabsWet_AtmosProf), q_lay(:,Iatm), Molecule_ID=1 )
    enddo      
  endif       
  ...
  WRITE(*,*) 'Calculate atmospheric predictors.'
  ...
  WRITE(*,*) 'Channel loop starts.'
  !--- top of channel loop

  Loop_Ichan: do Ichan = Ichan_top, Ichan_last

    WRITE(*,*)
    WRITE(*,*) 'Channel (Seq #, Ch #) : ', Ichan, channel_list(Ichan)
...
!    if(SpcCoeff%Is_Microwave_Channel(Ichan) == 1)then
    ...
    enddo
    enddo
    WRITE(*,*) 'Brightness Temperatures calculated.'
    !------------------------------------------------------------
    ...
    WRITE(*,*) 'Regression weights get calculated.'
    ...
    !--- get predictands
    WRITE(*,*) 'Predictors get calculated.'
    ...
    WRITE(*,*) ' Max abs(predictand) :', real( Max_Predictand, 4 )
    WRITE(*,*) ' Max atm term        :', real( Max_Predictand * Criterion_AtmPredSensitivity(Iabsorber), 4 )
    ...
    if( Nsample < Natm * Nangle * Nlay / 4 )then
      ...
      WRITE(*,*) '### PASS GENERATING COEFFICIENTS (NO ENOUGH PREDICTANDS AVAILABLE) ###'
      WRITE(*,*) '### ZERO ABSORPTION IS ASSUMED                                     ###'
      goto 1000
    endif
    ...
    if( all( atmpredflag_avail(:,Ichan) <= 0 ) )then
      Npredcomb(Ichan) = -1
      WRITE(*,*) '### PASS GENERATING COEFFICIENTS (ALL PREDICTORS CANNOT BE USED) ###'
      goto 1000
    endif
    ...
    Loop_polyOrder: do Isel = 1, Nselection_Polyorder
      WRITE(*,*) 'Loop over polynomial order.'
...
500   continue
      ...
      Loop_allcomb: do
        ...
        !--- check whether the atmos pred combination is sensitive
        ...
        WRITE(*,*) 'Generate coefficients and get statistics.'
      ...
      enddo Loop_allcomb
      WRITE(*,*) 'Loop_allcomb has finished.'
      ...
      if( MinIdx_AllComb >= INFINITE / TWO )then
        ...
        if( Natmpred_allcombsearch < 1) then
         ...
         WRITE(*,*) 'Natmpred_allcombsearch:', Natmpred_allcombsearch
         goto 500
        endif 
      endif
      ...
      ! the previous step (all combination search). When this step is done
...
!!          endif
    ...
    !==================================================================
      ...
      WRITE(*,*) 'Try prediction with no atmos pred.'
      call Gen_TransCoefSub( &
                     Ichan,  &
                     0, predcomb(1,Ichan),          &
                     Nsample,                       &
      ...
      if( Iflag == 0 .and. stat%index < MinIdx_Npred(Npredcomb(Ichan)) )then
        WRITE(*,*) '### PREDICTING WITH NO ATMOSPHERIC PREDICTOR IS BETTER ###'
        Npredcomb(Ichan)                   = 0
        predcomb(1:Natmpred_maxused,Ichan) = 0
    ...
    !==================================================================
      ...
      if( Iflag == 0 .and. stat%index < MinIdx_Npred(Npredcomb(Ichan)) )then
        WRITE(*,*) '### PREDICTING NO ABSORPTION IS BETTER ###'
        Npredcomb(Ichan)                   = -1
        predcomb(1:Natmpred_maxused,Ichan) = 0
        Npolyorder_chan(Ichan) = 0
      endif
...
1000 continue
    ...
    WRITE(*,*) '---> The best predictor set for channel', channel_list(Ichan), 'is'
    ...
    if( Iflag > 0 )then
      WRITE(*,*) '### ERROR ###'
      WRITE(*,*) 'SENSITIVE ATMOSPHERIC PREDICTOR SET IS SELECTED'
      stop 90
    endif
...
!   write( cformat, '("(i2,1h),",i1,"i3,F7.3,X,4F9.5,X,4F10.7,X,3F10.7,X,F10.7,X,F15.2,I2)")' ) Natmpred_maxused
    WRITE( *,* ) Npolyorder, Natmpred, dmy_predcomb(1:Natmpred_maxused), &
                  stat%residual, &
                  stat%tbmean,     stat%tbsderr,     stat%tbrmse,     stat%tbmaxerr, &
                  stat%taumean,    stat%tausderr,    stat%taurmse,    stat%taumaxerr, &
                  stat%taumaxmean, stat%taumaxsderr, stat%taumaxrmse, &
