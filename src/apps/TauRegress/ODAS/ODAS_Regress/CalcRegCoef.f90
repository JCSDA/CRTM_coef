...
!================================================================  
  ...
  subroutine DGELS_LAPACK(Nsample, Nel, x, y, coef, residual)
    ...
    !--- local variables
    !real(fp_kind)  :: xwork(Nsample, Nel) ! This leads to a stack overflow
    !(SIGSEV)
    REAL(fp_kind), ALLOCATABLE :: xwork(:,:)
    ...
    IF(.NOT. ALLOCATED(xwork) ) ALLOCATE(xwork(Nsample, Nel),STAT=alloc_stat)
    IF( alloc_stat /= 0 ) THEN 
      WRITE(*,*) 'DGELS_LAPACK: Error allocating array xwork.'
      STOP 99
    END IF 
    ...
    WRITE(*,*) 'DGELS Nsample: ', Nsample
    CALL DGELS('No transpose', Nsample, Nel, 1, xwork, Nsample, ywork, &
    ...
    DEALLOCATE(xwork)
    !WRITE(*,*) 'SIZE OF COEF: ', SHAPE(coef)
    !WRITE(*,*) 'SIZE of YOWRK: ', SHAPE(ywork)
    
    coef(:) = ywork(1:Nel, 1)
    residual = 0.0_fp_kind
    do i = 1, Nsample
