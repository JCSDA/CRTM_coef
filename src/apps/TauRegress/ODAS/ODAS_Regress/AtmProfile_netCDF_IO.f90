...
!
!       Modified by:    Isaac Moradi, 16-Jun-2016
...
MODULE AtmProfile_netCDF_IO
  ...
  CHARACTER(*), PARAMETER :: MODULE_VERSION_ID = &
    '$Id: AtmProfile_netCDF_IO.f90 32969 2013-10-30 14:20:41Z paul.vandelst@noaa.gov $'
...
!------------------------------------------------------------------------------
  ...
  RESULT( err_stat )
    ...
    err_stat = AtmProfile_netCDF_InquireFile( &
                 Filename, &
                 n_Profiles = n_profiles )
    WRITE(*,*) 'Number of profiles available in the netCDF file: ', n_profiles
    ! WRITE(*,*) 'AtmProfile Inquiry successful.'
    IF ( err_stat /= SUCCESS ) THEN
    ...
    WRITE(*,*) fileid, n, groupid
    nf90_stat = NF90_INQ_GRPS( fileid, n, groupid )
    WRITE(*,*) nf90_stat, n, groupid
    IF ( nf90_stat /= NF90_NOERR ) THEN
    ...
    Profile_loop: DO n = 1, n_profiles
      ...
      IF ( ReverseProfile ) THEN
         ! Level data
        k = AtmProfile(n)%n_Layers
        print*, k, AtmProfile(n)%n_Layers
        AtmProfile(n)%Level_Pressure(0:k)    = AtmProfile(n)%Level_Pressure(k:0:-1)
        AtmProfile(n)%Level_Temperature(0:k) = AtmProfile(n)%Level_Temperature(k:0:-1)
        AtmProfile(n)%Level_Absorber(0:k,:)  = AtmProfile(n)%Level_Absorber(k:0:-1,:)
        AtmProfile(n)%Level_Altitude(0:k)    = AtmProfile(n)%Level_Altitude(k:0:-1)
        ! Layer data
        AtmProfile(n)%Layer_Pressure(1:k)    = AtmProfile(n)%Layer_Pressure(k:1:-1)
        AtmProfile(n)%Layer_Temperature(1:k) = AtmProfile(n)%Layer_Temperature(k:1:-1)
        AtmProfile(n)%Layer_Absorber(1:k,:)  = AtmProfile(n)%Layer_Absorber(k:1:-1,:)
        AtmProfile(n)%Layer_Delta_Z(1:k)     = AtmProfile(n)%Layer_Delta_Z(k:1:-1)
      END IF
