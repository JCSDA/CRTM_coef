...
PROGRAM Create_ProcessControl_File
  ...
  WRITE( *, FMT     = '( /5x, "Enter a SensorInfo filename: " )', &
            ADVANCE = 'NO' )
  ! I changed the read to getting this as an argument - Changed by Isaac
  CALL GET_COMMAND_ARGUMENT(1, SensorInfo_Filename)
  ...
  ! I changed it from READ to GET_ARG - Isaac Moradi
  ! Used SRF_FilePath as temporary value holder
  ! See Tau_Production_Parameters.f90 - it should be 2 for AIRS and MISTiC
  CALL GET_COMMAND_ARGUMENT(2, SRF_FilePath)
  READ(SRF_FilePath, *) dF_Index  ! Convert string to INTEGER
  ...
  WRITE( *, FMT     = '( /5x, "Enter the SRF data file path [e.g. ./SRF_Data/]: " )', &
            ADVANCE = 'NO' )
  ! Changed it from READ to GET_ARG - Isaac Moradi
  CALL GET_COMMAND_ARGUMENT(3, SRF_FilePath)
  ...
  Sensor_Count_Loop: DO n = 1, n_Sensors
    ...
    SRF_Filename = TRIM(TRIM( SRF_FilePath )//TRIM(ADJUSTL(SensorInfo%Sensor_Id))//'.osrf.nc')
...
!    END IF
    ...
    Frequency_Interval_Loop: DO ch = 1, SensorInfo%n_Channels
      ...
      ! This is a temporary fix - df = FREQUENCY_INTERVAL(dF_Index) - Changed by Isaac
      ! df = FREQUENCY_INTERVAL(dF_Index) 

      ! -- Compute the frequency interval index - This was commented out but I changed it to calculate df_Indx - Isaac 
    ...
    Channel_Loop: DO ch = 1, SensorInfo%n_Channels
      ...
      ! -------------------------------------------
      WRITE(*,*)  Begin_SRF_Frequency(ch), FREQUENCY_BEGIN
      WRITE(*,*)  End_SRF_Frequency(ch), FREQUENCY_END
      ...
      !     (   End_SRF_Frequency(ch) <= FREQUENCY_END   )       ) THEN
        ...
        WRITE(*,*) "Channel Index: ", l, SensorInfo%n_Channels
        ...
        ProcessControl%List(l)%End_LBLband   = Compute_LBL_band( End_SRF_Frequency(ch), &
                                                                 ProcessControl%dF_Index(n)  )
        ! Compute_LBL_band Doesn't work currently but I know that I only have 1 band for 
        ! MISTiC instrument - Added by Isaac Moradi
        ! ProcessControl%List(l)%Begin_LBLband = 1
        ! ProcessControl%List(l)%End_LBLband = 1
  ...
  WRITE( *, FMT     = '( /5x, "Enter the Process Control output filename : " )', &
            ADVANCE = 'NO' )
  ! Changed the following to GET_ARG - Isaac Moradi
  CALL GET_COMMAND_ARGUMENT(4, ProcessControl_Filename)
