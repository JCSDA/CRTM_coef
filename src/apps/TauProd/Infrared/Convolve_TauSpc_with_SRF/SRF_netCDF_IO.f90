...
MODULE SRF_netCDF_IO
  ...
  '$Id: SRF_netCDF_IO.f90 5182 2009-10-13 14:57:44Z yong.chen@noaa.gov $'
  ! Invalid flag
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE CreateNames( Channel          , &  ! Input
    ...
    IF ( PRESENT(Point_DimName    ) ) Point_DimName     = 'ch'//TRIM(c)//'_b1_n_Points'
    IF ( PRESENT(Band_DimName     ) ) Band_DimName      = 'ch'//TRIM(c)//'_n_Bands'
    IF ( PRESENT(Response_VarName ) ) Response_VarName  = 'ch'//TRIM(c)//'_response'
    IF ( PRESENT(f1_Band_VarName  ) ) f1_Band_VarName   = 'ch'//TRIM(c)//'_f1_band'
    IF ( PRESENT(f2_Band_VarName  ) ) f2_Band_VarName   = 'ch'//TRIM(c)//'_f2_band'
    IF ( PRESENT(npts_Band_VarName) ) npts_Band_VarName = 'ch'//TRIM(c)//'_npts_band'
