...
MODULE Tau_Production_Parameters
  ...
  !USE LBLRTM_File_Define, ONLY: LBLRTM_FILE_TYPE => LBLRTM_SINGLE_PANEL_TYPE
  ...
  INTEGER, PUBLIC, PARAMETER, DIMENSION(N_FREQUENCY_INTERVALS) :: &
    MAX_N_LBLBANDS = (/ 12, & ! For 0.1cm^-1 spacing (was 6, changed to 12 for viirs-m_n21)
                       120, & ! For 0.0025cm^-1 spacing (was 10, changed to 120)
                        1 /) ! For 0.001cm^-1 spacing
...
!       17, &  !  wvd  
  ...
  INTEGER, PUBLIC, PARAMETER ::           RADIANCE    = 3 ! Chnaged it from 0 to 3 to match with DIRECTION_NAME  - Isaac Moradi may 2016
