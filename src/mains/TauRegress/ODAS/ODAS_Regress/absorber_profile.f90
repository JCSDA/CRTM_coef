...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Absorber_Amount( Pressure,    &  ! Input,  K
    ...
    !IF(dP > 0.) then
      !WRITE(*,*) 'Pressure difference dP is positive.'
    !END IF 
    ...
    k_layer_loop: DO k = 2, SIZE( Pressure )
      ...
      !IF(dP > 0.) then
        !WRITE(*,*) 'Pressure difference dP is positive.', k
      !END IF 
...
!-------------------------------------------------------------------------------
!
! $Id: absorber_profile.f90 4099 2009-06-17 19:12:03Z yong.chen@noaa.gov $
!
