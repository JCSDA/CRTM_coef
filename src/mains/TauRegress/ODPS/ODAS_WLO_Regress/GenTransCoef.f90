...
! Modified by P. Stegmann, Nov. 20, 2020, Restored code functionality.
!
!==============================================================================
...
program GenTransCoef
  ...
  '$Id: GenTransCoef.f90 18896 2020-11-20 18:22:00Z patrick.stegmann@noaa.gov $'
  ...
  REAL(fp_kind), ALLOCATABLE ::  p_lev_reverse(:)
  REAL(fp_kind), ALLOCATABLE ::  layer_reversal(:)
...
1200 continue
  ...
  ALLOCATE( p_lev_reverse (Nlay+1) )
  ALLOCATE( layer_reversal (Nlay))
  allocate( p_lay (  Nlay) )
  ...
  !
  !  PS_comment: atmospheric profiles are assigned to global variables
  !              and reversed at the same time. 
  !
  p_lev(0:NLay) = AtmProfile(1)%Level_Pressure
  p_lay(:)      = AtmProfile(1)%Layer_Pressure
  profile_loop: DO Iatm = 1,Natm
    !WRITE(*,*) 'Set abs arrays from AtmProfiles.', Iatm
    !p_lev_reverse       = AtmProfile(Iatm)%Level_Pressure(0:Nlay)
    !p_lev(0:Nlay,Iatm)  = p_lev_reverse((Nlay+1):1:-1) ! Reverse atmosphere
    !p_lev(0:Nlay,Iatm)   = AtmProfile(Iatm)%Level_Pressure(0:Nlay)
    !layer_reversal      = AtmProfile(Iatm)%Layer_Pressure(:)
    !p_lay(:,Iatm)       = layer_reversal(Nlay:1:-1)
    !p_lay(:,Iatm)        = AtmProfile(Iatm)%Layer_Pressure(:)
    !layer_reversal      = AtmProfile(Iatm)%Layer_Temperature(:)
    !t_lay(:,Iatm)       = layer_reversal(Nlay:1:-1)
    t_lay(:,Iatm)        = AtmProfile(Iatm)%Layer_Temperature(:)
    !WRITE(*,*) P_lev(0:NLay,:)
    !layer_reversal      = AtmProfile(Iatm)%Layer_Absorber(:,IabsOzn_AtmosProf)
    !o3_lay(:,Iatm)      = layer_reversal(Nlay:1:-1)
  ...
  if( Absorber_Units_ID(IabsWet_AtmosProf) == UnitID_HITRAN_gpkg )then
    DO Iatm = 1,Natm
      !layer_reversal = AtmProfile(Iatm)%Layer_Absorber(:,IabsWet_AtmosProf)
      !q_lay(:,Iatm) = layer_reversal(Nlay:1:-1)
...
!    endif
    ...
    !WRITE(*,*) 'wettrans_lev: ', wettrans_lev
...
1000 continue
    ...
    WRITE(*,*) 'Ncoef: ', Ncoef, SIZE(coef(:,Ichan))
