...
PROGRAM Compute_Coeff
  ...
  USE ODPS_Predictor, ONLY : ODPS_Get_max_n_Predictors,  &  
                             ODPS_Get_n_Components,      & 
                             ODPS_Get_n_Absorbers,       & 
                             ODPS_Get_Component_ID,      &  ! Component IDs
                             ODPS_Get_Absorber_ID,       &  ! Absorber IDs
...
!  USE ODPS_Binary_IO, ONLY : Write_ODPS_Binary
  ...
  CHARACTER( * ), PARAMETER :: PROGRAM_RCS_ID = &
    '$Id: Compute_Coeff.f90 19791 2012-06-12 15:20:53Z paul.vandelst@noaa.gov $'
  ...
  INTEGER :: This_Group
  CHARACTER(LEN=1) :: This_Group_in ! For command line input as char.
  ...
  TYPE( ODPS_Predictor_type )  :: Predictor
  ...
  LOGICAL :: isUseful 
  ! Read in Group ID (Use for debugging)
  !WRITE(*, *)'Enter Group ID: 1,2 or 3'
  !READ(*, '(i5)')This_Group

  ! Get Group ID as Command Line Argument
  ! as Compute_Coeff gets called from
  ! run_tau_coeff.sh shell script:
  CALL GET_COMMAND_ARGUMENT(1,This_Group_in)
  READ(This_Group_in,'(i1)') This_Group
  ...
  Ncom = ODPS_Get_n_Components(This_Group) 
  ...
  ! Reversed inFileName_atmProfile and AtmProfile argument,
  ! deleted: Reverse = .TRUE.
  ...
  ! Inquire number of layers from AtmProfile:
  ...
  Nabs = ODPS_Get_n_Absorbers(This_Group)
  NsensorChan = SpcCoeff%n_Channels
  n_predictors_max = ODPS_Get_max_n_Predictors(This_Group)
  
  ! get component IDs
  Component_ID = ODPS_Get_Component_ID(icom, This_Group)
  ...
  !#--------------------------------------------------------------------------#  
  !#              -- Set the absorbers and their IDs and Indices  --          #           
  !#--------------------------------------------------------------------------#  
  DO i = 1, Nabs
   absorber_id(i) = ODPS_Get_Absorber_ID(i, This_Group)
  ...
  WRITE(*,*) 'Reference profiles are calculated.'
  ...
  Ref_Sum_Loop: DO m = 1, Natm
    Ref_Level_Pressure(0) = ref_level_pressure(0) + AtmProfile(m)%Level_Pressure(0)
    DO k = 1, Nlay
!      Ref_Level_Pressure(k) = ref_level_pressure(k) + AtmProfile(m)%Level_Pressure(k+1)
      Ref_Level_Pressure(k) = ref_level_pressure(k) + AtmProfile(m)%Level_Pressure(k)
  ...
  ! ------------------------------------------------------------
  ! Changes: put Predictor as first argument.
  CALL ODPS_Predictor_Create( Predictor,   &
                                        nLay, &
                                        nLay, &
                                        Ncom,   &
                                        n_Predictors_max )
  ...
  Compute_Predictor_Loop: DO m = 1, Natm
    ...
    DO i = 1, n_Angles
      CALL ODPS_Compute_Predictor(This_Group, Temperature, Absorber,   &
...
!                STOP 90
    ...
    WRITE(*,*) 'Calculating brightness temperatures.'
...
!**** PvD: Not sure the dimension changes here will work!!! ****
    ...
    DEALLOCATE(  Tau, TauTotal, TauPred, TauTotalPred, TauFactor, tb_pred, tb_lbl, &        
               STAT = Allocate_Status )                                                      
  ...
  CALL ODPS_Predictor_Destroy(Predictor)
  IF ( Allocate_Status /= SUCCESS ) THEN
    ...
    DO i = 1,  Nangle_regression    ! Loop over scan angles                                           
      DO m= 1, Natm   ! Loop over profiles                                                          
        ...
        DO k = 1, Nlay     ! Loop over layers                                                       
          ...
          IF(Component_ID /= ODPS_Get_Ozone_Component_ID(This_Group) .AND. OD < ZERO)OD = ZERO
  ...
  SUBROUTINE Compute_BrightTemp(ichan, n_angles, n_profiles, tau, &
    ...
    DO i = 1,  n_Angles     ! Loop over scan angles
      DO m= 1, n_profiles   ! Loop over profiles   
        n_levels = atmprofile(m)%n_Layers
