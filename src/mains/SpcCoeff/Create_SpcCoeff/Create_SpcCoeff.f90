...
! =====           =======           ============
! 2021-07-14      P. Stegmann       Modified for hyperspectral MW
!                                   instruments such as ATMS-NG.   
!
!
...
PROGRAM Create_SpcCoeff
  ...
  '$Id: Create_SpcCoeff.f90 46292 2021-01-05 19:48:19Z patrick.stegmann@noaa.gov $'
  ! Keyword set flag
  ...
  END IF
  ! PS: Increased spccoeff%Version from 2 to 3.
  ! This accounts for the added polarization angle. 
  !spccoeff_file_version = 3
  ...
  SELECT CASE( sensor_type )
    CASE( MICROWAVE_SENSOR )
      ...
      !spccoeff%PolAngle = mw_sensordata%PolAngle
      !spccoeff%Polarization = HL_POLARIZATION
      OPEN(UNIT = 444, FILE="polarization.txt",STATUS='OLD',ACTION='READ')
      polarization_loop: DO ii = 1, n_channels
        READ(444, *) spccoeff%Polarization(ii)
      END DO polarization_loop
      CLOSE(444)
      IF( spccoeff%Version > 2 ) THEN
        OPEN(UNIT = 555, FILE="polangle.txt",STATUS='OLD',ACTION='READ')
        angle_loop: DO ii = 1, n_channels
          READ(555, *) spccoeff%PolAngle(ii)
        END DO angle_loop
        CLOSE(555)
      END IF
      !CALL MW_SensorData_DefineVersion( MW_SensorData_History )
      MW_SensorData_History = 'Test implementation with Create_SpcCoeff; '//TRIM(MW_SensorData_History)
