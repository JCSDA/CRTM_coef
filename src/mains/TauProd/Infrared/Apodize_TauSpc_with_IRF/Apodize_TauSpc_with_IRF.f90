...
!
! Update: Added IASI-NG support. (PS)
!
! CREATION HISTORY:
!       Written by:     Paul van Delst, CIMSS/SSEC 27-Feb-2007
!                       paul.vandelst@ssec.wisc.edu
!       Modified by:    Patrick Stegmann, JCSDA 24-Sept-2020
!       
...
! =====           =======                  ============
! 2020-09-24      Patrick Stegmann         Restored functionality of code
! 2021-02-24      Patrick Stegmann         Added IASI-NG parameters
!
!
...
PROGRAM Apodize_TauSpc_with_IRF
  ...
  USE LBLRTM_netCDF_IO         , ONLY: LBLRTM_netCDF_InquireFile, &
                                       LBLRTM_netCDF_ReadFile, &
                                       LBLRTM_netCDF_WriteFile
  USE SPC_IFG_Utility          , ONLY: CosFilter, &
                                       ComputeNPoints, &
                                       ComputeIndex, &
                                       ComputeNextPO2, &
                                       ComputeNSPC, &
  ...
  USE IASI_Define              , ONLY: IASI_MIN_FREQUENCY, &
                                       ...
                                       IASI_ApodFunction, &
                                       IASI_Channels
  USE IASI_NG_Define           , ONLY: IASI_NG_MIN_FREQUENCY, &
                                       IASI_NG_MAX_FREQUENCY, &
                                       IASI_NG_D_FREQUENCY, &
                                       IASI_NG_RESAMPLE_MAXX, &
                                       N_IASI_NG_CHANNELS, &
                                       N_IASI_NG_BANDS, &
                                       IASI_NG_BAND_F1, &
                                       IASI_NG_BAND_F2, &
  ...
  '$Id: Apodize_TauSpc_with_IRF.f90 21071 2021-02-24 21:48:00Z patrick.stegmann@noaa.gov $'
  ! Rollof filter width
...
!  REAL(fp), PARAMETER :: F_NYQUIST = MASTER_F2
  ...
  INTEGER,  PARAMETER,  DIMENSION(N_SENSOR_SETS) :: WMO_SATELLITE_ID = (/4, 224, 840/)   ! PS: Added 840
  REAL(fp), PARAMETER,  DIMENSION(N_SENSOR_SETS) :: MASTER_F1 = (/ZERO, ZERO, ZERO/)     ! PS: Added ZERO
  REAL(fp), PARAMETER,  DIMENSION(N_SENSOR_SETS) :: MASTER_F2 = (/6912.0_fp, 6400.0_fp, 3200.0_fp/) ! PS: OPD = 4cm
  REAL(fp), PARAMETER,  DIMENSION(N_SENSOR_SETS) :: F_NYQUIST = MASTER_F2
  INTEGER,  PARAMETER,  DIMENSION(N_SENSOR_SETS) :: N_BANDS = (/3, 3, 4/) ! PS: Added 4
  ...
  CHARACTER(256) :: ioBuffer
  CHARACTER(256) :: Character_buffer
  ...
  TYPE(LBLRTM_File_type) :: ofile ! Added by Patrick
  ...
  CALL Program_Message( PROGRAM_NAME, &
                        ...
                        'back into the spectral domain.',&
                        'Test version P. Stegmann, 2020-2021.')
  ...
  !READ(*,'(a)') LBL_Filename
  CALL GET_COMMAND_ARGUMENT(1,Character_buffer)
  READ(Character_buffer,*) LBL_Filename
  LBL_Filename = ADJUSTL(LBL_Filename)

  ! Inquire the file
  !Error_Status = LBLRTM_netCDF_InquireFile( LBL_Filename                     , &
  ...
  Error_Status = LBLRTM_netCDF_ReadFile(ofile, LBL_Filename)
  
  bf = MINVAL(ofile%Layer(:)%Begin_Frequency)
  ef = MAXVAL(ofile%Layer(:)%End_Frequency) 

  n_lbl_layers = 100
  dirn = UPWELLING_DIRECTION
  df=0.001
  n_ispc = 900000  
  ...
  !  STOP
  !END IF
  CALL GET_COMMAND_ARGUMENT(2, Character_buffer)
  READ(Character_buffer,*) iProfile_Set
  ...
  !  STOP
  !END IF
  CALL GET_COMMAND_ARGUMENT(3, Character_buffer)
  READ(Character_buffer,*) iMolecule
  ...
  !  STOP
  !END IF
  CALL GET_COMMAND_ARGUMENT(4, Character_buffer,IO_Status)
  READ(Character_buffer,*) iProfile
  ...
  !  STOP
  !END IF
  CALL GET_COMMAND_ARGUMENT(5, Character_buffer)
  READ(Character_buffer,*) iAngle
  ...
  !  STOP
  !END IF
  CALL GET_COMMAND_ARGUMENT(6, Character_buffer)
  READ(Character_buffer,*) iDir
  ...
  CALL GET_COMMAND_ARGUMENT(7, Character_buffer)
  READ(Character_buffer,*) iSensor
  !IF ( IO_Status /= 0 ) THEN
  !  CALL Display_Message( PROGRAM_NAME, &
  ...
  !  STOP
  !END IF
  CALL GET_COMMAND_ARGUMENT(8,Character_buffer)
  READ(Character_buffer,*) iBand
  ...
  x = ComputeX(f)
  write (*, *) 'Size of TIFG and SPC: ', size(x), size(f)
  ...
  Sensor_Select: SELECT CASE (iSensor)
   ...
   CASE( 3 ) ! PS
    it0 = MINLOC(x, DIM=1, MASK=(x == ZERO ) )
    it1 = MINLOC(x, DIM=1, MASK=(x > -IASI_NG_RESAMPLE_MAXX(iBand)))
    it2 = MAXLOC(x, DIM=1, MASK=(x <= IASI_NG_RESAMPLE_MAXX(iBand)))
  ...
  SELECT CASE (iSensor)
   ...
   CASE (3) ! PS
    irf = IASI_NG_ApodFunction(iBand,x)
    ! --------------------------------------------
    ! Determine the actual channel numbers to keep
    ! --------------------------------------------
    ic1 = INT((IASI_NG_BAND_F1(iBand) - f1)/IASI_NG_D_FREQUENCY(iBand) + ONEpointFIVE)
    ic2 = INT((IASI_NG_BAND_F2(iBand) - f1)/IASI_NG_D_FREQUENCY(iBand) + ONEpointFIVE)
  END SELECT   
  ...
  SELECT CASE (iSensor)
   ...
   CASE (3)
     realTau%Channel        = IASI_NG_Channels(iBand)
  END SELECT
  ...
  Layer_loop: DO iLayer = 1, N_LAYERS
    ...
    ! ---------------------------------------------
    !Error_Status = LBLRTM_netCDF_ReadFile( LBL_Filename, &
    !                                   iLayer, &
    !                                   Transmittance=spc(is1:is2) )
    Error_Status = LBLRTM_netCDF_ReadFile( ofile, &
                                           LBL_Filename)
    IF ( Error_Status /= SUCCESS ) THEN
      CALL Display_Message( PROGRAM_NAME, &
    ...
    spc(is1:is2) = ofile%Layer(iLayer)%Spectrum(:,1)
