...
! =====       =======           ============
! 2021-11-01  P. Stegmann       Changed frequency units from cm^-1 to GHz
!                               (Central_Frequency, f1, f2, etc.).
!
...
MODULE oSRF_File_Define
  ...
  '$Id: oSRF_File_Define.f90 45754 2014-09-17 17:28:50Z paul.vandelst@noaa.gov $'
  ! Invalid flag
  ...
  CHARACTER(*), PARAMETER:: FREQUENCY_NAME             = 'Gigahertz [GHz]'
  CHARACTER(*), PARAMETER:: WAVENUMBER_NAME            = 'Inverse centimeters [cm^-1]'
  CHARACTER(*), PARAMETER:: WAVELENGTH_NAME            = 'Micrometers [mum]'
  ...
  CHARACTER(LEN=SL) :: FREQUENCY_UNITS = WAVENUMBER_NAME 
...
!------------------------------------------------------------------------------
  ...
  RESULT( err_stat )
    ...
    Channel_Loop: DO n = 1, n_channels
      ...
      CALL oSRF_Destroy( oSRF )
      DEALLOCATE( n_points, & ! PS: temperature, &
                  STAT = alloc_stat, ERRMSG = alloc_msg )
      IF ( alloc_stat /= 0 ) THEN
...
!------------------------------------------------------------------------------
  ...
  RESULT( err_stat )  
    ...
    nc_stat(3) = NF90_PUT_ATT( fileid, varid, UNITS_ATTNAME      , TRIM(ADJUSTL(FREQUENCY_UNITS))) 
    ...
    Define_Channel_Loop: DO n = 1, self%n_Channels
      ...
      FREQUENCY_UNITS = WAVENUMBER_NAME
      !IF ( self%Sensor_Type == MICROWAVE_SENSOR ) THEN
      !  FREQUENCY_UNITS = FREQUENCY_NAME
      !ELSE IF ( self%Sensor_Type == INFRARED_SENSOR ) THEN
      !  FREQUENCY_UNITS = WAVENUMBER_NAME
      !ELSE IF ( self%Sensor_Type == VISIBLE_SENSOR ) THEN
      !  FREQUENCY_UNITS = WAVELENGTH_NAME
      !ELSE IF ( self%Sensor_Type == ULTRAVIOLET_SENSOR ) THEN
      !  FREQUENCY_UNITS = WAVELENGTH_NAME
      !ELSE
      !  msg = '("Invalid oSRF_File Sensor_Type)'
      !  err_stat = -1
      !  CALL Display_Message(ROUTINE_NAME, msg, err_stat) 
      !  CALL Cleanup(); RETURN
      !END IF 
