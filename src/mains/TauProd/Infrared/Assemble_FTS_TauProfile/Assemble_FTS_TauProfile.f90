...
!                       Patrick Stegmann, 2020-09-30
!                       patrick.stegmann@noaa.gov
!
!
PROGRAM Assemble_FTS_TauProfile
  ...
  REAL(fp), ALLOCATABLE :: Geometric_Angle_Fix(:,:,:) ! A fix for the missing angle.
  ...
  !CALL GET_COMMAND_ARGUMENT(1,Characterbuffer)
  !READ(Characterbuffer,*) iProfile_Set
  IF ( IO_Status /= 0 ) THEN
  ...
  !CALL GET_COMMAND_ARGUMENT(2,Characterbuffer)
  !READ(Characterbuffer,*) n_j
  IF ( IO_Status /= 0 ) THEN
    CALL Display_Message( PROGRAM_NAME, &
                          'Invalid input.', &
                          FAILURE )
  ...
  !CALL GET_COMMAND_ARGUMENT(3,Characterbuffer)
  !READ(Characterbuffer,*) Molecule_Set_Numbers(1:n_j)
  IF ( IO_Status /= 0 ) THEN
    CALL Display_Message( PROGRAM_NAME, &
  ...
  !CALL GET_COMMAND_ARGUMENT(4,Characterbuffer)
  !READ(Characterbuffer,*) iDir
  IF ( IO_Status /= 0 ) THEN
    CALL Display_Message( PROGRAM_NAME, &
  ...
  !CALL GET_COMMAND_ARGUMENT(5,Characterbuffer)
  !READ(Characterbuffer,*) n1
  !CALL GET_COMMAND_ARGUMENT(6,Characterbuffer)
  !READ(Characterbuffer,*) n2
  IF ( IO_Status /= 0 .OR. n1 > n2  ) THEN
    WRITE(*,*) n1, n2
    CALL Display_Message( PROGRAM_NAME, &
                          'Invalid BAND input.', &
                          FAILURE )
  ...
  Band_Loop: DO n = n1, n2
    ...
    ALLOCATE(Geometric_Angle_Fix(N_LAYERS, N_ZENITH_ANGLES, N_PROFILES(iProfile_Set)))
    ...
    Molecule_Loop: DO jIdx = 1, n_j
      ...
      Profile_Loop: DO m = m1, m2
        ...
        Angle_Loop: DO i = i1, i2
          ...
          ! -----------------------------------
          !WRITE(*,*) TauProfile%Tau(:,:,1,1,1)
          !WRITE(*,*) PATH//TRIM(OutFile)
          ...
          ! This is the Geometric_Angle fix for ODPS
    ...
    ! Write Geometric Angle - This was missing in the original code so there was
    ! some weird TauCoef problem. 
    Error_Status = Write_GeometricAngle_netCDF(PATH//TRIM(OutFile), &
                   Geometric_Angle_Fix)
    IF ( Error_Status /= SUCCESS ) THEN
         CALL Display_Message ( PROGRAM_NAME, &
                                'Error writing geometric angle from '//&
                                TRIM( InFile )//&
                                ' to ' //TRIM(OutFile), &
                                Error_Status)
         STOP
    ...
    DEALLOCATE(Geometric_Angle_Fix)
