...
PROGRAM Assemble_TauProfile_Data
  ...
  CHARACTER( * ),  PARAMETER, DIMENSION( 2 ) :: DIRECTION__NAME = (/ 'upwelling  ', &
                                                                    'downwelling' /)
  ...
  WRITE( *, FMT     = '( /5x, "Select atmospheric path", &
                       ...
                       &/10x, "2) ", a, &
                       &/5x, "Enter choice: " )', &
            ADVANCE = 'NO' ) DIRECTION__NAME( 1 ), DIRECTION__NAME( 2 )
  READ( *, FMT    = '( i5 )', &
...
! Replaced this because level pressure is reversed in the Tau_Production_Parameter
                                                     ...
                                                     WMO_Sensor_ID    = WMO_Sensor_ID, &
                                                     ID_Tag  = TRIM(ID_Tag), &
                                                     Title   = TRIM(DIRECTION__NAME( Direction ))//&
                                                               ' transmittances for '//&
                                                               TRIM(Sensor_Id), &
          ...
          ! ------------------------------------------
          !print*, "TauProfile_Filename", ProcessControl%TauProfile_Filename( n )
          !print*, "ZENITH_ANGLE_SECANT", ZENITH_ANGLE_SECANT(i)

        
          ! Here are the dimenstions of some of the variables
          ! Transmittance profiles  
          ! REAL(fp), POINTER :: Tau(:,:,:,:,:)    => NULL() ! K x L x I x M x J  OR n_Layers  x Channel x n_Angles x Profile x Molecule_Set 
    ...
    ! Write Geometric Angle - This was missing in the original code so there was some wierd TauCoef
    ! I am adding it here for test  - Isaac Moradi
    Error_Status = Write_GeometricAngle_netCDF(TRIM( ProcessControl%TauProfile_Filename( n ) ), &
                                Geometric_Angle_Fix)
