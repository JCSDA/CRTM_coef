...
!                       Yong.Chen@noaa.gov
!
!       Modified by:    Isaac Moradi, ESSIC/GMAO/NASA 25-May-2016
...
PROGRAM Convolve_TauSpc
  ...
  ! Module usage
  ! ------------
  USE LBLRTMIO_Module  ! Added by Isaac
  USE Type_Kinds
  USE File_Utility
  USE Message_Handler
...
!  USE oSRF_Define
  ...
  '$Id: Convolve_TauSpc.f90 11527 2016-5-25 19:30:10Z isaac.moradi@nasa.gov $'
...
!                                                                 'downwelling' /)
  ...
  REAL(fp), ALLOCATABLE :: Transmittance_Band(:,:) ! The first index is just like  Transmittance(:) and the second is for bands - Isaac Moradi
...
!  TYPE(oSRF_type), ALLOCATABLE :: SRF(:)
  ...
  TYPE(LBLRTM_File_type) :: ofile  ! Added by Isaac
...
!  END IF
  ...
  READ(Control_Filename , *) Profile_Set
  ...
  CALL GET_COMMAND_ARGUMENT(2, Control_Filename)
  READ(Control_Filename , *) Molecule_Set_Number
...
!  END IF
  ...
  CALL GET_COMMAND_ARGUMENT(3, Control_Filename)
  READ(Control_Filename , *) Profile_Number
...
!    STOP
!  END IF
  CALL GET_COMMAND_ARGUMENT(4, Control_Filename)
  READ(Control_Filename , *) Angle_Number
  ...
  WRITE( *, FMT     = '( /5x, "Enter the Frequency intervals index: " )', &
            ADVANCE = 'NO' )
  CALL GET_COMMAND_ARGUMENT(5, Control_Filename)
  READ(Control_Filename, *) dF_Index
...
!    STOP
!  END IF
  CALL GET_COMMAND_ARGUMENT(6, Control_Filename)
  READ(Control_Filename, *) Direction
...
!  READ( *, FMT = '( a )' ) Control_Filename
  CALL GET_COMMAND_ARGUMENT(7, Control_Filename)
  ...
  Initialize_TauProfile_Loop: DO n = 1, ProcessControl%n_Files
    ...
    ProcessControl%File_Prefix(n) = TRIM(ADJUSTL(ProcessControl%File_Prefix(n)))
    ...
    ! SRF filename... simple
    ProcessControl%SRF_Filename(n) = TRIM(TRIM(ADJUSTL( ProcessControl%File_Prefix( n ) ))//&
                                     '.osrf.nc')
    ...
    ProcessControl%TauProfile_Filename(n) = TRIM(TRIM( DIRECTION_NAME( Direction ) )//&
...
!      dF_Index = Compute_dF_Index( dF )
  ...
  WRITE(*,*) "MaxBand, MinBand: ", Max_Band, Band1
  ...
  Main_Processing_Loop: DO
    ...
    WRITE(*,*) "Starting Band: ", Band2, Band1
    ...
    Band1_Increment: DO
      ...
      ! upper limit.
      IF ( Band1 > Band2 ) THEN
        WRITE(*,*) "Main Loop is cycled.", Band2, Band1
        CYCLE Main_Processing_Loop
      END IF
    ...
    Check_Data_Availability: IF ( n_Available_Bands == n_Bands .AND. &
      ...
      ! Allocate new variable - added by Isaac Moradi
      ALLOCATE( Transmittance_Band( n_Spectral_Points, N_LAYERS ), &
                STAT = Allocate_Status )
      ...
      Band_Frequency_Limit_Loop: DO lBand = 1, n_Available_Bands
        ...
        ! I have added the following lines to read LBLRTM NetCDF file then calculate the minimum of v1 and v2
        ! The old code only reads the tau file layer by layer but that code doesn't exist anymore in the svn
        ! I also changed the LBRTM_netCDF_Read to read the file all at once then put it in the Transmittance
        ! variable. Sample file is in    
        ! Tau_Filename is like profile12/angle1/all_withcontinua/band001/upwelling_tau.profile12_angle1_all_withcontinua_band001.nc.signal
        Error_Status = LBLRTM_netCDF_ReadFile(ofile,Tau_Filename(Band_Index(lBand)) )

        ! Get the current data file frequency limits
        ! and comment global attribute
! Changed by Isaac Moradi
...
!                                              Comment = History )
        ...
        vb1 = MINVAL(ofile%Layer(:)%Begin_Frequency)
        vb2 = MAXVAL(ofile%Layer(:)%End_Frequency)
        ! End of code added by Isaac Moradi
...
!      ALLOCATE( SRF( n_Channels_to_Process ), &
      ...
      !#------------------------------------------------------------------------#
      !#       -- READ ALL THE BANDS BEFORE LOOPING OVER LAYERS --              #
      !#               Added by Isaac Moradi
      !#------------------------------------------------------------------------#
      ! -----------------------------
      ...
      Band_File_Read_LBL: DO lBand = 1, n_Available_Bands
        ...
        ! Read the LBLRTM TauFiles into an array to avoid re-reading the data in the next loop
        ! Tau_Filename is like profile12/angle1/all_withcontinua/band001/upwelling_tau.profile12_angle1_all_withcontinua_band001.nc.signal
        Error_Status = LBLRTM_netCDF_ReadFile(ofile,Tau_Filename(lb) )
        IF ( Error_Status /= SUCCESS ) THEN
        ...
        ! Assign the data lay-by-layer to Transmittance_Band
        Process_Layer_Read_LBL: DO k = 1, N_LAYERS
           Transmittance_Band(l1:l2,k) = ofile%Layer(k)%Spectrum(:(l2-l1+1),1)
        END DO Process_Layer_READ_LBL

        ! Update the begin index
        l1 = l2 + 1

      END DO Band_File_Read_LBL
      ...
      Process_Layer: DO k = 1, N_LAYERS
        ...
        Band_File_Read: DO lBand = 1, n_Available_Bands
          ...
          ! Added by Isaac Moradi to assign the values from previous loop
          ! Not the best way to do this but didn't want to mess around
          Transmittance(l1:l2) = Transmittance_Band(l1:l2,k)
...
!      DEALLOCATE( SRF, SRF_Index, STAT = Allocate_Status )
      ...
      DEALLOCATE( Transmittance, Transmittance_Band, &
                  STAT = Allocate_Status )
