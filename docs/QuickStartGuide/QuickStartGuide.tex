...
In order to create a new case directory, run the helper from \verb|workflows/MW/|:
\begin{verbatim}
 python3 Create_CaseDirectory.py -i <instrument_name>
\end{verbatim}
where \verb|instrument_name| is the CRTM instrument identifier. This will create the case directory.\\
For the concrete example of TROPICS this would be:
\begin{verbatim}
 python3 Create_CaseDirectory.py -i tropics_sv1_srf_v2
\end{verbatim}
For help on the input options type:
\begin{verbatim}
 python3 Create_CaseDirectory.py -h
\end{verbatim}
Once the case folder is created, you can enter it to proceed.
\begin{verbatim}
 cd tropics_sv1_srf_v2/
\end{verbatim}
If you installed binaries into a \verb|bin/| folder, you can add it to \verb|PATH| or symlink the executables you need. If you built with spack-stack, executables are located under \verb|build-spack/src/apps/<area>/<app>/<app>|; you can run them directly via full paths.
...
The CRTM oSRF file is an intermediate representation of the SRF as a netCDF file. The converter application is \verb|oSRF_Create_from_ASCII| (in \verb|build-spack/src/apps/oSRF/oSRF_Create_from_ASCII/| when built with spack-stack).
Place your SRF ASCII files in \verb|SRFDATA/| and run the tool from the case directory. For example, for \verb|cosmir_air| (1 band, 9 channels, no interpolation):
\begin{verbatim}
...
no
\end{verbatim}
This will create the file \verb|cosmir_air.osrf.nc|.  You can use interpolation if desired, but many MW cases work without it.

\subsection{Creating the SpcCoeff file}
Creating the SpcCoeff file requires the \verb|polangle.txt| and \verb|polarization.txt| files in the case directory (see examples in \verb|workflows/MW/cosmir_air/|).
...
After you've created these files, your working directory (e.g., \verb|workflows/MW/<sensor>|) should contain:
\begin{verbatim}
SRFDATA/
polangle.txt
polarization.txt
...
First, link or copy the \verb|ECMWF83.AtmProfile.nc| predictor dataset from \verb|data/reference/TauProd/AtmProfile/|.
...
This step can be run locally (single process) or via batch systems. An example case directory for the regression is provided in \verb|workflows/MW/GenCoeff/|.
...
Here's a local example using the spack-stack build (adjust \verb|<repo_root>| and \verb|<build_dir>|):
\begin{verbatim}
:MAX_CPUs:1
:CH_INT:9
:EXE_FILE:<build_dir>/src/apps/TauRegress/ODPS/ODPS_Regress/ODPS_Regress
:WORK_DIR:<repo_root>/workflows/MW/<sensor>
:PROF_SET:ECMWF83
:SPC_COEFF_DIR:<repo_root>/workflows/MW/<sensor>
:TAU_PROFILE_DIR:<repo_root>/workflows/MW/<sensor>
:ATM_PROFLE_FILE:<repo_root>/workflows/MW/<sensor>/ECMWF83.AtmProfile.nc
:GET_SEN_INFO:<build_dir>/src/apps/TauRegress/ODPS/GetSenInfo/GetSenInfo
:IR_TYPE:3
:COMPONENTS:DEFAULT
#COMPONENTS:ozo
...
For a local run, you can run the helper script directly from your case directory:
\begin{verbatim}
../../scripts/gen_tau_coeff_local.sh tau_coeff.parameters.local
bash ./cat_taucoef.sh tau_coeff.parameters.local <build_dir>/src/apps/TauRegress/ODPS/Assemble_ODPS/Assemble_ODPS
\end{verbatim}
Note: \verb|cat_taucoef.sh| uses bash-specific syntax.

If everything worked, the resulting TauCoeff file can be found in the \verb|results/| folder.  If it seems to hang, check for missing libraries or absent \verb|Completion_signal.txt| files in the execution directories.
