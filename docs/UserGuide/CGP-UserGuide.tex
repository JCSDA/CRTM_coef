...
The name of the Github repository is \verb CRTM_coef  and it consists of the following core folders;
\begin{itemize}
\item \begin{verbatim} src/ \end{verbatim}
\item \begin{verbatim} docs/ \end{verbatim}
\item \begin{verbatim} workflows/ \end{verbatim}
\item \begin{verbatim} data/reference/ \end{verbatim}
\item \begin{verbatim} staging/ \end{verbatim}
\end{itemize}
The \verb src/  folder contains all the Fortran source code of this package, organized by application under \verb src/apps  for Step 1 (\verb TauProd ) and Step 2 (\verb TauRegress ) of the coefficient generation process.
The \verb docs/  folder contains the documentation pertaining to the package. The \verb workflows/  folder provides sample working directories for running all steps of the coefficient generation process in one place. The working directory in turn contains two subfolders called \verb TauCoeffTest  and \verb GenCoeff  to separate Step 1 and Step 2 of the process work.
The \verb data/reference/  folder contains shared reference data (e.g., atmospheric profiles and solar spectra). The \verb staging/  folder is used to hold experimental changes and new coefficients awaiting review.
 \begin{figure}[H]
 \centering
...
\begin{verbatim}
$ cd src/apps/TauProd/AtmProfile/AtmProfile_Create
$ make -f Makefile
\end{verbatim}
...
\begin{enumerate}
\item Enter the directory \verb src/apps/TauProd/Microwave/MW_TauProfile/ .
...
\begin{verbatim}
$ ln - s data/reference/ECMWF83.AtmProfile.nc ./ 
\end{verbatim}
\item Link in the instrument oSRF file, e.g. for AMSU-A:
\begin{verbatim}
$ ln -s data/reference/amsua_n15.osrf.nc ./
\end{verbatim}
...
\begin{verbatim}
$ cd src/apps/TauProd/Infrared/Create_LBLRTM_Input_Files	# Enter the folder
...
\begin{verbatim}
$ cd ..
$ ln -s TAPE5_files/ workflows/IR/TauCoeffTest/
\end{verbatim}

\paragraph{Preparation of the working directory}
The preceding two steps have shown how to prepare the \verb TAPE3  and \verb TAPE5  input files for the LBLRTM calculations. The subsequent preparation steps have largely already been carried out in the example \verb workflows/  of the \verb CRTM_coef  repository and are listed here to enable the users to setup their own line-by-line calculations.\\
...
\begin{verbatim}
$ cd src/apps/TauProd/Infrared/Create_ProcessControl_File/
...
This section is focused on the \textbf{ODPS} regression algorithm alone. The default working directory for the IR regression is \verb workflows/IR/GenCoeff .
\begin{verbatim}
$ cd workflows/Infrared/GenCoeff/
\end{verbatim}
...
#EXE_file=../../../src/apps/TauRegress/ODPS/Assemble_ODPS/Cat_ODPS
...
This concludes the transmittance coefficient training process. The final coefficient file will be available in NetCDF format and can be converted to CRTM native format using one of the utility programs, such as\\ \verb src/apps/TauRegress/ODPS/ODPS_NC2BIN .
