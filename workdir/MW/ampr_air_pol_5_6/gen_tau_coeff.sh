...
#  jobs_sensor=`echo $jobs_sensor $Ngas | awk '{print ($1 * $2)}'`
  ...
  for GasName in $GasName_List; do
    ...
    while [ $TopSeqCh -le $EndChan ]; do
      ...
      mkdir -p $RunDir
      cd ${RunDir}
      ln -s /data/users/bjohnson/CRTM/tmp3/CRTM_coef/workdir/MW/ampr_air/upwelling.ampr_air.TauProfile.nc ./
...
#      echo "#SBATCH --account=star" >> ${jobScript}
      ...
      echo "#SBATCH --output=/data/users/bjohnson/CRTM/tmp3/CRTM_coef/workdir/MW/ampr_air/control/output.%j.txt" >> ${jobScript}
      echo "#SBATCH --error=/data/users/bjohnson/CRTM/tmp3/CRTM_coef/workdir/MW/ampr_air/control/output.%j.err" >> ${jobScript}
