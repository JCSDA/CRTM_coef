...
#  jobs_sensor=`echo $jobs_sensor $Ngas | awk '{print ($1 * $2)}'`
  ...
  for GasName in $GasName_List; do
    ...
    while [ $TopSeqCh -le $EndChan ]; do
      ...
      mkdir -p $RunDir
      cd ${RunDir}
      ln -s /data/users/bjohnson/CRTM/tmp3/CRTM_coef/workdir/MW/tropics_sv5/upwelling.tropics_sv5.TauProfile.nc ./
...
EOF
     ...
     #--- move back to where the batch files are
      ...
      jobScript=CoeffJob_${TotalJob_count}.scr                               

      Error_file_id=${SatSen}.${GasName}.ch${TopSeqCh}
 
      echo "#!/bin/bash" >> ${jobScript}
      echo "#SBATCH --job-name=ODPS_Run.${Job_count}.out" >> ${jobScript}
...
#      echo "#SBATCH --account=star" >> ${jobScript}
      ...
      echo "#SBATCH --output=/data/users/bjohnson/CRTM/tmp3/CRTM_coef/workdir/MW/tropics_sv5/control/output.%j.txt" >> ${jobScript}
      echo "#SBATCH --error=/data/users/bjohnson/CRTM/tmp3/CRTM_coef/workdir/MW/tropics_sv5/control/output.%j.err" >> ${jobScript}
...
#      echo "module load hdf hdf5 netcdf4" >> ${jobScript}
			echo "module load netcdf-fortran" >> ${jobScript}
			echo "module load openblas" >> ${jobScript}
			echo "export LD_LIBRARY_PATH=/data/users/bjohnson/CRTM/tmp3/CRTM_coef/build/lib/:$LD_LIBRARY_PATH" >> ${jobScript}
